...
# Base Ariel workload
# Initial support: Pin and PEBIL
# Planned support: Miranda, Whatever that trace based thing is, and Pinballs
class Workload(object):
    def __init__(self):
        pass

#Generic keys
# verbose, corecount, checkaddresses, maxissuepercycle,
# maxcorequeue, maxtranscore, cachelinesize, mallocmapfile,
# writepayloadtrace, clock, tracegen, tracePrefix, memmgr

# NOTE - defaults should be parsed from sst-info, if available.

# Base class for both Pin and PEBIL workloads
# Should not be used by itself
class ExecutableWorkload(Workload):
    def __init__(self):
        Workload.__init__(self)
        self.envParams = {}

    def updateEnvParams(self, params):
        self.envParams.update(params)

    def deleteEnvParam(self, key):
        self.envParams.update(params)

# Execution specific keys
# profilefunctions (Pin only?), pipetimeout, arieltool, launcher, exectuable,
# appstdin, appstdout, appstderr, appstdoutappend, appstderrappend, launchparamcount,
# launchparam%, mpimode, mpilauncher, mpiranks, mpitracerank, envparamcount,
# envparamname%, envparamval%, appargcount, apparg%, arielmode, arielinterceptcalls,
# arielstack

class PinWorkload(ExecutableWorkload):
    def __init__(self):
        ExecutableWorkload.__init__(self)
        # TODO - check PIN is installed
        # TODO - check app can be found, is executable

class PebilWorkload(ExecutableWorkload):
    def __init__(self):
        ExecutableWorkload.__init__(self)
        # TODO - check app is instrumented

class PatternWorkload(Workload):
    def __init__(self):
        Workload.__init__(self)
        raise NotImplementedError("Ariel does not yet support a pattern-based frontend")

class TraceWorkload(Workload):
    def __init__(self):
        Workload.__init__(self)
        raise NotImplementedError("Ariel does not yet support a trace-based frontend")

class PinballWorkload(Workload):
    def __init__(self):
        Workload.__init__(self)
        raise NotImplementedError("Ariel does not yet support pinballs")
...
class TrafficGenEndPoint(EndPoint):
    ...
    def prepParams(self):
        ...
        if _params["PacketDest.pattern"] == "NearestNeighbor":
            self.nicKeys.append("PacketDest.NearestNeighbor.Size")
            if not "PacketDest.NearestNeighbor.Size" in _params:
                _params["PacketDest.NearestNeighbor.Size"] = "%s %s %s"%(_params["PacketDest.3D shape X"], _params["PacketDest.3D shape Y"], _params["PacketDest.3D shape Z"])
