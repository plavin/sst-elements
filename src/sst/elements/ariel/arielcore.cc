...
ArielCore::ArielCore(ComponentId_t id, ArielTunnel *tunnel,
    ...
    pendingTransStartTime = new std::unordered_map<StandardMem::Request::id_t, uint64_t>();
    ...
    statLatency = registerStatistic<uint64_t>("latency", subID);
...
void ArielCore::commitReadEvent(const uint64_t address,
    ...
    if(length > 0) {
        ...
        if(isGpuEx()){
            ...
            pendingTransStartTime->insert( std::pair<StandardMem::Request::id_t, uint64_t>(req->getID(), getCurrentSimTime()) );
#ifdef HAVE_CUDA
        }
#endif
        if(enableTracing) {
...
void ArielCore::handleEvent(StandardMem::Request* event) {
    ...
    if(find_entry != pendingTransactions->end()) {
        ...
        auto start_time = pendingTransStartTime->find(mev_id);
        if (start_time == pendingTransStartTime->end()) {
            output->fatal(CALL_INFO, -4, "Memory event response to core: %" PRIu32 " was not found in pendingTransStartTime.\n", coreID);
        }
        statLatency->addData(getCurrentSimTime() - start_time->second);
        //std::cout << "PATRICK: " << getCurrentSimTime() - start_time->second << std::endl;

        pendingTransactions->erase(find_entry);
        pendingTransStartTime->erase(start_time);
        pending_transaction_count--;
...
bool ArielCore::refillQueue() {
    ...
    while(coreQ->size() < maxQLength) {
        ...
        ArielCore::PhaseData *phase_data;
        StandardMem::CustomReq *req;
        switch(ac.command) {
...
void ArielCore::handlePhaseChange(ArielPhaseEvent* pEv) {
    ARIEL_CORE_VERBOSE(4, output->verbose(CALL_INFO, 4, 0, "Core %" PRIu32 " processing a phase event...\n", coreID));
    commitPhaseEvent(pEv->getPhase());
}
...
bool ArielCore::processNextEvent() {
    ...
    switch(nextEvent->getEventType()) {
        ...
        case PHASE_CHANGE:
                ...
                removeEvent = true;
                handlePhaseChange(dynamic_cast<ArielPhaseEvent*>(nextEvent));
